...
#             # Generar imágenes únicas para cada sección principal
#             print(f"Generando imágenes para: {titulo}")
#             for section in sections['slides']:
#                 if len(images) >= 4:  # Límite de 4 imágenes
#                     break
                
#                 # Generar una imagen considerando tanto el título principal como la sección
#                 context_prompt = f"{titulo} - {section['title']}"
#                 image_path = await get_image_for_presentation(context_prompt)
#                 if image_path:
...
#                 else:
#                     slide_layout = prs.slide_layouts[random.choice([1, 7, 8, 9])]
